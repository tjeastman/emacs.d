...
=olivetti-mode= automatically balances window margins.
#+begin_src emacs-lisp :tangle yes :comments link
...
=which-function-mode= is a minor mode that displays the current function name in the mode line.
#+BEGIN_SRC emacs-lisp :tangle yes :comments link
...
=combobulate= uses =treesit= to provide structured editing and movement for a wide range of programming languages.
#+BEGIN_SRC emacs-lisp :tangle yes :comments link
